...
| `CHALLENGED` | reject | feeder-local work allowed; any feeder can close and restart only its own PDA | before/equal `settle_by`, with count+digest match → `SETTLED`; re-execution chooses payout and completed feeder receives 10% slash reward | reject | after `settle_by` → `SETTLED`; challenger receives the complete pot | reject |
